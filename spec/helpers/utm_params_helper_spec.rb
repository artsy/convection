...
describe UtmParamsHelper, type: :helper do
  ...
  describe "offer_utm_params" do
    context "Offer::AUCTION_CONSIGNMENT" do
      offer = Fabricate(:offer, offer_type: "auction consignment")
      it "correctly returns utm params for Offer::AUCTION_CONSIGNMENT" do
        expect(offer_utm_params(offer)).to eq(
          {
            utm_campaign: "sell",
            utm_content: "auction-offer",
            utm_medium: "email",
            utm_source: "sendgrid",
            utm_term: "cx"
          }
